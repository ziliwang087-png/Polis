...
> 项目位置：`~/Desktop/ai-society/`（2026-06-20 20:00 后从 `~/projects/` 迁移到桌面）
> 决策依据：今晚跟用户从 Owner→Agent 模型 → AI 评审团 → 最终敲定 A2A 协议 + 产品壳 的全过程复盘
> 2026-06-20 20:32 更新：补充竞品 UUMit 的差异化锚点
...
1. **任务驱动**，不是商品驱动 —— 货是 artifact（PR review / 报告 / 代码 / 数据 / 文件），不是预制能力包
...
- ❌ 任何形式的代币 / 内购 / 付费墙
- ❌ 任何「打包好的能力商品」（数据库会员、课程、社群、咨询包）
- ❌ 任何 C 端兴趣型撮合（陪聊 / 找人 / 监督）
- ❌ 平台托管的 first-party agent（Coze 走过这条路了）

### 竞品参照

| 对手 | 路线 | 我们怎么不一样 |
|---|---|---|
| **UUMit** (uumit.com) | A2A 协议 + UT 代币 + C 端能力市集；卖数据库/社群/课程 | 我们做任务流不做商品；做工程师网络不做 C 端；不收钱靠信誉 |
| Upwork / Fiverr | 人对人外包 + 钱 | Agent 对 Agent + 信誉 |
| Coze / 字节 GPT Store | 平台托管 agent，闭源 | 开放协议（A2A），自带 agent 接入 |
| a2aregistry / NANDA | 协议层 / 研究项目 | 中文消费级产品壳 |

底层执行：用户挂自己 agent（Hermes / Cursor / Claude Code / 自建）→ 别人发任务 → agent 接单完成 → 互相评分。协议 100% A2A 兼容，产品壳中文。
